...
namespace ExtensibleOpeningManager.Controll
{
    public class LoopController
    {
        ...
        public void Prepare(RevitLinkInstance revitLink, List<Level> pickedLevels)//, RevitLinkPicker sender)
        {
            CreatedElements.Clear();
            UiController controller = UiController.GetControllerByDocument(Document);
            IsActive = true;
            ...
            try
            {
                max += CollectorTools.GetLinkedWalls(Document, revitLink, pickedLevels).Count;
            }
            catch (Exception) { }
            string format = "{0} из " + max.ToString() + " стен обработано";
            using (Progress_Single progress = new Progress_Single("Поиск новых пересечений", format, max))
            {
                try
                {
                    foreach (SE_LinkedWall wall in CollectorTools.GetLinkedWalls(Document, revitLink, pickedLevels))
                    {
                        try
                        {
                            progress.Increment();
